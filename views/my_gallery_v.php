...
echo("<h1>".$_SESSION['username']." gallery</h1>");

if (isset($_SESSION['lst_img']) && !empty($_SESSION['lst_img']) && isset($_SESSION['user_log']) && !empty($_SESSION['user_log'])) {
	...
	if ($pdo != NULL) {
		...
		$nb_page = get_nb_page_preview($pdo, 11, $_SESSION['user_id']);
		$npage = (int)$_GET['npage'];

		if (!isset($npage) || empty($npage) || ($npage < 1 || $npage > $nb_page)) {
			$npage = 1;
			ft_location('page', 'my_gallery&npage=1');
		} else {
			$uri = $_SERVER['REQUEST_URI'];
			if (strncmp($uri, '/index.php?page=', 16) == 0) {
				if (substr_compare($uri, 'my_gallery', 16, 10) == 0) {
		...
		<?php
			...
			if (isset($_GET['delete']) && isset($_GET['img_id']) && isset($_SESSION['user_log']) && !empty($_SESSION['user_log'])) {
				if (img_can_be_delete($pdo, $_SESSION['user_id'], $_GET['img_id']) === true) {
					delete_img($pdo, $_GET['img_id'], get_imgname_by_id($pdo, $_GET['img_id']));
					ft_location('page', 'my_gallery&npage='.$npage);
				}
			}
		...
		while ($count < 11 && isset($_SESSION['lst_img'][$count + (($npage - 1) * 11)]) && !empty($_SESSION['lst_img'][$count + (($npage - 1) * 11)]) && $_SESSION['lst_img'][$count + (($npage - 1) * 11)] != NULL) {
			...
			<span class="img_gallery">
				<form action="http://localhost:8080/index.php?page=my_gallery&npage=<? echo $npage?>&img_id=<?php echo($id_img); ?> " method="post">
					...
					<?php
						if (img_can_be_delete($pdo, $_SESSION['user_id'], $id_img) === true) {
							echo '<a href="http://localhost:8080/index.php?page=my_gallery&npage='.$npage.'&img_id='.$id_img.'&delete=true"><img class="img_delete" src="img/icons/delete.png"></a>';
						}
					?>
					<div display="inline-block"><a href="http://localhost:8080/index.php?page=my_gallery&npage=<? echo $npage;?>&img_id=<?php echo($id_img); ?>&like=true"><img class="img_like" src=" <?php echo $like ?>" /></a> <?php echo (get_nb_like($pdo, $id_img)); ?></div>
					...
					<textarea id="comment_input" class="img_comment_entry" name="comment" placeholder="Comment here" maxlength="255"></textarea>
